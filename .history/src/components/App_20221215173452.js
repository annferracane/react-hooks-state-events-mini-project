...
function App() {
  ...
  function onClickHandler(e) {
    const newCategoryButtons = categoryButtons.map(categoryButton => {
      console.log(categoryButton.category)
      if(e.target.id === categoryButton.id) {
        return <button key={categoryButton.category} id={categoryButton.id} className="selected" onClick={(e) => onClickHandler(e)}>{categoryButton.value}</button>;
      } else {
        return <button key={categoryButton.category} id={categoryButton.id} className="" onClick={(e) => onClickHandler(e)}>{categoryButton.value}</button>;
      }
    });
