...
function App() {
  ...
  function onClickHandler(e) {
    const newCategoryButtons = categoryButtons.map(categoryButton => {
      console.log(categoryButton.key);
      console.log(e);
      if(e.target === categoryButton) {
        return <button key={categoryButton.key} id={categoryButton.key} className="" onClick={(e) => onClickHandler(e)}>{category}</button>;
      } else {
        return <button key={categoryButton.key} id={category} className="" onClick={(e) => onClickHandler(e)}>{category}</button>;
      }
    });
