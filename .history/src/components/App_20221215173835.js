...
function App() {
  ...
  function buildCategoryButtonArray(categoryClass = "") {
    return CATEGORIES.map(category => {
      return <button key={category} id={category} className={categoryClass} onClick={(e) => onClickHandler(e)}>{category}</button>
    });
  }
  ...
  const [categoryButtons, setCategoryButtons] = useState(buildCategoryButtonArray);
